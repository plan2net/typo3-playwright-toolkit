...
# variables itself, so the add-on documents them and ships nothing. Hermetic.
...
@test "documents both connect variables Playwright reads" {
    ...
    grep -q 'ws://playwright-server:3000/' "${ADDON_DIR}/README.md"
}
...
@test "names the pin as the consumer's decision, not a default" {
    grep -q 'platform: linux/amd64' "${ADDON_DIR}/README.md"
}
